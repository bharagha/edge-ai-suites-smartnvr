...
class FrigateService:
    ...
    def get_camera_names(self) -> list:
        ...
        try:
            ...
            raise HTTPException(
                status_code=502,
                detail=f"Failed to connect to Frigate: {str(e)}"
            )
    ...
    def get_camera_clip(
        ...
        try:
            ...
            if e.response.status_code == 404:
                raise HTTPException(
                    status_code=404,
                    detail="No clip found for specified time range"
                )
            raise
    ...
    def _validate_time_range(start_time: int, end_time: int):
        ...
        if (end_time - start_time) > 300:  # 5 minute limit
            raise HTTPException(
                status_code=400,
                detail="Clip duration cannot exceed 300 seconds"
            )
    async def get_event_clip(self, event_id: str, download: bool = False) -> StreamingResponse:
        ...
        try:
            ...
            if e.response.status_code == 404:
                raise HTTPException(
                    status_code=404,
                    detail=f"Clip not found for event ID: {event_id}"
                )
            raise HTTPException(
                status_code=502,
                detail=f"Frigate error: {e}"
            )
        except requests.exceptions.RequestException as e:
            raise HTTPException(
                status_code=502,
    ...
    async def export_camera_clip(
            ...
            try:
                ...
                raise HTTPException(
                    status_code=502,
                    detail=f"Failed to contact Frigate: {str(e)}"
                )
    ...
    async def stream_export_video(self, export_id: str, download: bool = False) -> FileResponse:
    ...
    def get_clip_from_timestamps(
        ...
        try:
            ...
            if e.response.status_code == 404:
                raise HTTPException(status_code=404, detail="Clip not found for specified time range")
            raise HTTPException(status_code=502, detail=f"Frigate error: {e.response.text}")
        except requests.exceptions.RequestException as e:
