...
def process_video(camera_name: str, start_time_input: Union[str, float, datetime, None], end_time_input: Union[str, float, datetime, None], action: str, label: str = None) -> Dict:
    ...
    def parse_input_to_timestamp(input_val, label: str) -> Tuple[Optional[float], Optional[str]]:
        """
        ...
        try:
            ...
            # Handle ISO format strings
                ...
                if dt.tzinfo is None:
                    logger.debug(f"Naive datetime from ISO string for {label}, assuming IST")
                    dt = IST.localize(dt)
                else:
            ...
            logger.debug(f"Converted {label} to IST: {readable} (timestamp: {timestamp})")
    ...
    start_time, start_time_str = parse_input_to_timestamp(start_time_input, "start_time")
    ...
    if start_time >= end_time:
        return {"status": "error", "message": "Start time must be earlier than end time"}

    try:
        if action == "Summarize":
            logger.info(f"Initiating summarization for {camera_name} from {start_time_str} to {end_time_str}")
...
def create_ui():
    ...
    with gr.Blocks(title="NVR Event Router", theme=theme) as interface:
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("AI-Powered Event Viewer"):
                ...
                def fetch_and_display_events(camera_name):
                    ...
                    if camera_name:
                        ...
                        logger.info(f"Fetched events in {fetch_time:.2f}s, displaying {len(recent_events)} events")
                        formatted_events = display_events()
                        
                        # Ensure we're returning a list of lists
                        if not isinstance(formatted_events, list):
                            logger.error(f"Unexpected return type from display_events: {type(formatted_events)}")
                            return []
                        if formatted_events and not isinstance(formatted_events[0], list):
                            logger.error(f"Unexpected inner type from display_events: {type(formatted_events[0])}")
                            return []
                ...
                camera_events_dropdown.change(
                    fn=lambda camera: (logger.info(f"Camera changed to: {camera}"), fetch_and_display_events(camera))[1],
            ...
            with gr.TabItem("Summarize/Search Clips"):
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=1):
                        process_button = gr.Button("Process Video", scale=0, min_width=100)
            ...
            with gr.TabItem("Auto-Route Events to AI Search"):
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=1):
                        process_auto_button = gr.Button("Add Rule", scale=0, min_width=100)
            
        
        # Process button handler
        process_button.click(
            fn=lambda *args: (logger.info(f"Process button clicked with args: {args}"), process_video(*args)),
            inputs=[camera_dropdown, start_time_input, end_time_input, action_dropdown],
            outputs=[process_result]
        )
        ...
        process_auto_button.click(
            fn=lambda *args: (logger.info(f"Auto-route button clicked with args: {args}"), process_video(*args)),
